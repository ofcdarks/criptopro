...
function initHFTPanel() {
  ...
  loadHFTAnalytics();
    updateMaxTradesHint();
    // Restaura trail settings salvos no banco (se existirem)
    if (hft.trail_l1) {
      ['l1','l2','l3','l4','l5','l6'].forEach(k => {
        const val = hft['trail_' + k];
        if (val) {
      ...
      // Desmarca presets (usuário tinha config custom salva)
      Object.keys(TRAIL_PRESETS).forEach(n => {
        const btn = document.getElementById('trail-preset-' + n);
        if (btn) { btn.style.border = '1px solid var(--t3)'; btn.style.background = 'transparent'; btn.style.color = 'var(--t2)'; btn.style.fontWeight = '400'; }
      });
      // Verifica se bate com algum preset pra marcar ele
      Object.entries(TRAIL_PRESETS).forEach(([name, p]) => {
        if (String(p.l1) === String(hft.trail_l1) && String(p.l2) === String(hft.trail_l2) &&
            String(p.l3) === String(hft.trail_l3) && String(p.l4) === String(hft.trail_l4)) {
          const btn = document.getElementById('trail-preset-' + name);
          if (btn) { btn.style.border = '1px solid rgba(83,63,171,.7)'; btn.style.background = 'rgba(83,63,171,.18)'; btn.style.color = 'var(--purple,#8b7fff)'; btn.style.fontWeight = '700'; }
        }
      });
      updateTrailPreview();
    } else {
      // Primeira vez: aplica preset padrão
      setTimeout(() => { applyTrailPreset('equilibrado'); updateTrailPreview(); }, 50);
    }
  });
  // Inicializa AI preview
  setTimeout(() => {
...
const TRAIL_PRESETS = {
  conservador: { l1: 0.22, l2: 0.35, l3: 0.55, l4: 0.85, l5: 1.30, l6: 2.00 },
  equilibrado: { l1: 0.28, l2: 0.45, l3: 0.70, l4: 1.10, l5: 1.60, l6: 2.50 },
  agressivo:   { l1: 0.35, l2: 0.55, l3: 0.90, l4: 1.40, l5: 2.20, l6: 3.50 },
...
function updateTrailPreview() {
  const l1 = parseFloat(document.getElementById('trail-l1')?.value || 0.28);
  const l2 = parseFloat(document.getElementById('trail-l2')?.value || 0.45);
  const l3 = parseFloat(document.getElementById('trail-l3')?.value || 0.70);
  const l4 = parseFloat(document.getElementById('trail-l4')?.value || 1.10);
  const l5 = parseFloat(document.getElementById('trail-l5')?.value || 1.60);
  ...
  el.innerHTML =
    ...
    ` <b style="color:#1D9E75"> | L2 ${l2.toFixed(2)}%</b> → trava 30%` +
    ` <b style="color:#BA7517"> | L3 ${l3.toFixed(2)}%</b> → trava 50%` +
    ` <b style="color:#D85A30"> | L4 ${l4.toFixed(2)}%</b> → trava 65%` +
    ` <b style="color:#A855F7"> | L5 ${l5.toFixed(2)}%</b> → trava 75%` +
    ` <b style="color:#EC4899"> | L6 ${l6.toFixed(2)}%</b> → trava 80% (trailing)`;
}

function getTrailConfig() {
  return {
    hft_trail_l1: document.getElementById('trail-l1')?.value || '0.28',
    hft_trail_l2: document.getElementById('trail-l2')?.value || '0.45',
    hft_trail_l3: document.getElementById('trail-l3')?.value || '0.70',
    hft_trail_l4: document.getElementById('trail-l4')?.value || '1.10',
    hft_trail_l5: document.getElementById('trail-l5')?.value || '1.60',
...
function updateHFTProfitPreview() {
  ...
  const sl       = parseFloat(document.getElementById('hft-sl-pct')?.value    || '0.45');
...
async function saveHFTSettings() {
  try {
    ...
    const hft = {
      ...
      trail_l3:   trailCfg.hft_trail_l3 || '0.15',
      trail_l4:   trailCfg.hft_trail_l4 || '0.28',
      trail_l5:   trailCfg.hft_trail_l5 || '0.50',
      trail_l6:   trailCfg.hft_trail_l6 || '1.10',
    };
...
async function hftStart() {
  ...
  const config = {
    ...
    hft_tp_pct:     document.getElementById('hft-tp-pct')?.value     || '0.45',
  ...
  const trailInfo = parseFloat(config.hft_trail_l1 || '0.28') < 5.00
...
function _hftRenderTrades(trades, pagination) {
  ...
  const rows = trades.map(t => {
    ...
    if (trailMatch) {
      const colors = { '1':'#378ADD','2':'#1D9E75','3':'#BA7517','4':'#D85A30','5':'#A855F7','6':'#EC4899' };
